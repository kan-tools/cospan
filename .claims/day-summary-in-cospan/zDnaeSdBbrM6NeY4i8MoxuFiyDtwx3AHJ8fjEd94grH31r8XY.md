---
{
  "v": 3,
  "cid": "bafyreibfpbz2igunf4sgiyqkttifc23n23p7j75gx2j5jesaq742nwvizq",
  "sig": "2d0a43abb822cf52ce10c069972c775550a71620aa70c266fe7d89d098a9f2ba5228c9b9edb8211a5329a422fa6e5caaa9a44fb98bc4708cd9f691145c85d51f",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "decision",
  "cites": [],
  "rev": "223mtflqg5pho",
  "seq": 0,
  "of": 8,
  "text_len": 593,
  "content": ""
}
---

cospan should render the compact day process-summary the harness statusline shows (`day status-line`): the 'atom · N/M done · next' line, the branch/clean/repo line, and the transition / off-sequence note. Near-term: add it as a compact summary header/pane in the cospan dashboard, beside the verbatim `day status`. Long-term goal: move this rendering OUT of the day harness statusline and INTO cospan, so cospan owns the process-summary display rather than day's hook. Relates to [[kan-is-truth]] (it's a projection of day's state) and the harness-view direction in .dropbox/05-views-ux.md.
***8<***
---
{
  "v": 3,
  "cid": "bafyreigaft4xvnrtdnmupk4wp6yyyd6gasyyehlezscl7wjursqlixzc3y",
  "sig": "100721fe0c42ba150ccd7d2c7b846886e3b204e7b89088555a9cd33f782494bc7c3f4c8efde3c726deaac19085f4f7159813e1c3bab8a93d007709eb890d5810",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "subject",
  "cites": [],
  "rev": "223mtflqg724q",
  "seq": 1,
  "of": 8,
  "content": ""
}
---
***8<***
---
{
  "v": 3,
  "cid": "bafyreievnn4j3hp4djgkebfyattidhg2d4jow4756s6ulvbd4ixd5ify3q",
  "sig": "6479b919b05633823a8595b678ace09a2ef0b1c457f9cbec503a9de67bb8880a0b308fb92d130d767f677372a686851a1d13abe18c35a8f4ce93e668e19f72f5",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "publication",
  "cites": [],
  "rev": "223mtflqgcgsc",
  "seq": 2,
  "of": 8,
  "content": ""
}
---
***8<***
---
{
  "v": 3,
  "cid": "bafyreifztvvue2akf46gsuuk24jarnf2f62uzmp6l6g7mmay6dzesevrx4",
  "sig": "f6b835ce85ef33928ffcc0b5ea37eca50c258e016da29953c6ad39cbfc7a1c5d5281220af17e45699d8f7b1c225ecff0a579891863fcbc0a6b3746059d299626",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "decision",
  "cites": [
    "bafyreigaft4xvnrtdnmupk4wp6yyyd6gasyyehlezscl7wjursqlixzc3y"
  ],
  "rev": "223mtflyantbf",
  "seq": 3,
  "of": 8,
  "text_len": 450,
  "content": ""
}
---

Deferred, blocked on upstream day. Sourcing the compact summary honestly needs a machine-readable day status: cospan must not parse day's human `day status` text (brittle) nor read the session-start `day status-line` cache (stale). It should consume structured output — e.g. a `day status --json` — which does not exist yet in the day repo (confirmed: `day status --json` errors, no --json flag). Revisit once day exposes machine-readable status.
***8<***
---
{
  "v": 3,
  "cid": "bafyreiaycuvscrxy7aoe6qgerpnfkagixq5oyoqgftxepbrowenvq2kpzy",
  "sig": "d31d7c915becbeccd0c1d417ea787acb5a449da15e02a96caeb0a309c6af83bd31d23d7b7b3de123e58423ee61160acbdc71f4eadc8d886a98d3eff6a9ad48ea",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "status",
  "cites": [
    "bafyreifztvvue2akf46gsuuk24jarnf2f62uzmp6l6g7mmay6dzesevrx4"
  ],
  "rev": "223mtflyap4jr",
  "seq": 4,
  "of": 8,
  "content": ""
}
---
***8<***
---
{
  "v": 3,
  "cid": "bafyreic7kwd37dakhk3wqamnacs4ub372wf6ehfhhnvxebgpfhq3xj4cxu",
  "sig": "624f69970b69c1ec1a353277c892769c3274879bb3f832763611097b880e5d63667e431c54fd5c0ffdeb6b1d9317b4b25cc816535fd643dc2aaf06861a1a4e8e",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "publication",
  "cites": [],
  "rev": "223mtflyaspbf",
  "seq": 5,
  "of": 8,
  "content": ""
}
---
***8<***
---
{
  "v": 3,
  "cid": "bafyreifvd4rxximz5wtprlf3622tunklr6hkr4bm545wpjcprdlawkekfi",
  "sig": "33ed743b74cf483f63a0093e00a324e614ce1d0970a103e28e58d4a1334aaa911b6fea3a20114a2a6af1e1f8fb3ddb3c4d0ea99172c9941a38c135099256af6e",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "observation",
  "cites": [],
  "rev": "223mtidrnrwqx",
  "seq": 6,
  "of": 8,
  "text_len": 196,
  "content": ""
}
---

Upstream unblock tracked: filed kan-tools/day#240 requesting machine-readable day status (--json) for the live atom position + per-witness state this subject needs. When that lands, this unblocks.
***8<***
---
{
  "v": 3,
  "cid": "bafyreiecao7roybe3doihyxjgh7473foxgmsna7m7r7lmjci52tccccfge",
  "sig": "3ab3f916c505bbe7dce29cec60a20799b07cc7a42bf2a06966c284041119daa554986e127da3a8912ede115202b7c5eac7e718f208c4ea71cef94ab1cff1fab1",
  "author": "did:key:zDnaeSdBbrM6NeY4i8MoxuFiyDtwx3AHJ8fjEd94grH31r8XY",
  "subject": {
    "local": "day-summary-in-cospan"
  },
  "kind": "publication",
  "cites": [],
  "rev": "223mtiewnjxnh",
  "seq": 7,
  "of": 8,
  "content": ""
}
---
